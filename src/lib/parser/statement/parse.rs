...
use std::{cell::RefCell, char, sync::Mutex};
...
fn is_variable_assignment_stmt<'a>(
    cmd: &'a str,
    captures_out: &mut Option<regex::Captures<'a>>,
) -> bool {
    let locked = VARIABLE_ASSIGNMENT_REGEX.lock().unwrap();
    ...
    let current_captures: Option<regex::Captures<'a>> =
        locked.borrow().as_ref().unwrap().captures(cmd);
    if let Some(current_inner) = current_captures {
        captures_out.replace(current_inner);
    }
    captures_out.is_some()
}

pub fn parse(code: &str) -> super::Result {
    let cmd = code.trim();
    let mut va_captures: Option<regex::Captures<'_>> = None;
    match cmd {
        ...
        _ if is_variable_assignment_stmt(cmd, &mut va_captures) => Ok(Statement::LocalAssignAct {
            key:   va_captures.as_ref().unwrap().get(1).unwrap().as_str().to_owned(),
            value: va_captures.as_ref().unwrap().get(2).unwrap().as_str().to_owned(),
            stmt:  Box::new(parse(va_captures.as_ref().unwrap().get(3).unwrap().as_str())?),
        }),
